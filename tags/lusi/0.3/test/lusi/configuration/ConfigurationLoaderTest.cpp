...
void ConfigurationLoaderTest::testLoad() {
    ...
    ConfigurationSaver().save(getPtr(configuration), localUrl);

    SmartPtr<ConfigurationParameterMap> loadedConfiguration(
                                                        loader.load(localUrl));
    ...
    CPPUNIT_ASSERT_THROW(loader.load(localUrl), PersistenceException);
    ...
    //Test with a inexistent file
    localUrl = LocalUrl(mTestDirectoryPath + "inexistentFile.xml");
